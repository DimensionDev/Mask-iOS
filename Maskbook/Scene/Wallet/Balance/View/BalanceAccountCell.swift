...
class BalanceAccountCell: UITableViewCell {
    ...
    private var sendView: ActionView = {
        let view = ActionView()
        view.config(image: Asset.Images.Scene.Balance.Button.send.image, title: L10n.Scene.WalletBalance.btnSend)
    ...
    private var receiveView: ActionView = {
        let view = ActionView()
        view.config(image: Asset.Images.Scene.Balance.Button.receive.image, title: L10n.Scene.WalletBalance.btnReceive)
        view.addLayoutConstraints()
        return view
    }()
    ...
    private var historyView: ActionView = {
        let view = ActionView()
        view.config(image: Asset.Images.Scene.Balance.Button.history.image, title: L10n.Scene.WalletDetail.walletItemsHistory)
        view.addLayoutConstraints()
        return view
    }()
    ...
    private func _init() {
        ...
        NSLayoutConstraint.activate([
            actionsStackView.topAnchor.constraint(
                equalTo: accountCardView.bottomAnchor,
                constant: 22
            ),
            actionsStackView.leadingAnchor.constraint(
                equalTo: accountCardView.leadingAnchor,
                constant: 12
            ),
            actionsStackView.trailingAnchor.constraint(
                equalTo: accountCardView.trailingAnchor,
                constant: -12
            ),
...
extension BalanceAccountCell: AccountCardViewDelegate {
    func moreButtonDidClick(view _: AccountCardView, button: UIButton) {
...
extension BalanceAccountCell {
    func showBalanceNotEnoughAlert() {
        ...
        guard let symbol = mainToken?.symbol.uppercased() else {
            return
        }
        ...
        Coordinator.main.present(
            scene: .alertController(alertController:
                alertController),
