//
//  SettingBackupPasswordViewController.swift
//  Maskbook
//
...
class ChangeBackupPasswordViewController: UIViewController {
    ...
    let subtitleLabel: UILabel = {
        ...
        label.text = L10n.Scene.ChangeBackupPassword.subTitle
    ...
    let passwordItemView: PasswordFormItemView = {
        let view = PasswordFormItemFactory.createBackupFormItem(autoVerify: true)
...
extension ChangeBackupPasswordViewController {
        ...
        confirmButton.cv.publisher(on: .touchUpInside).sink { [weak self] _ in
            self?.dismiss(animated: false) {
                self?.coordinator.present(
                    scene: .changeBackupPasswordStep2,
                    transition: .panModel()
                )
            }
        }.store(in: &disposeBag)
...
extension ChangeBackupPasswordViewController: KeyboardHandlerContent {
