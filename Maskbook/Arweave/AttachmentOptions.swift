...
struct AttachmentOptions {
    internal init(encrypted: Bool, type: String?, block: [UInt8], name: String) {
...
extension AttachmentOptions {
    ...
    func encryptedData() throws -> Data {
        if let passphrase = key {
            ...
            let fileData = try aes.encrypt(self.block)
            let algorithm = [
                "name": "AES-GCM",
                "iv": iv,
                "tagLength": 128
    ...
    func toTransaction() throws -> Transaction {
        ...
        var transaction = Transaction(data: encryptData)
        let tag = Transaction.Tag(name: "Content-Type", value: Self.defaultType)
