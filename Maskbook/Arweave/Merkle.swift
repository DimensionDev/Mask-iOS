...
struct MerkelTools {
    static let MAX_CHUNK_SIZE = 256 * 1024
    static let MIN_CHUNK_SIZE = 32 * 1024
    static let NOTE_SIZE = 32
    static let HASH_SIZE = 32
    
    static func chunkData(data: Data) -> [Chunk] {
        var chunks = [Chunk]()
        var rest = data
        var cursor = 0
        
        while rest.count >= Self.MAX_CHUNK_SIZE {
            
            var chunkSize = Self.MAX_CHUNK_SIZE
            
            let nextChunkSize = rest.count - Self.MAX_CHUNK_SIZE
    ...
    static func generateLeaves(chunks: [Chunk]) -> [LeafNode] {
    ...
    static func hashId(data: [Data]) -> Data {
    ...
    static func intToBuffer(note: Int) -> Data {
        var note = note
        var buffer = Data(capacity: Self.NOTE_SIZE)
        
        for _ in stride(from: Self.NOTE_SIZE - 1, through: 0, by: -1) {
            let byte = note % 256
    ...
    static func buildLayers(nodes: [MerkelNode], level: Int = 0) -> MerkelNode {
        if nodes.count < 2 {
            return nodes[0]
        }
    ...
    static func generateTransactionChunks(data: Data) -> Chunks {
    ...
    static func generateProofs(root: MerkelNode) -> [Proof] {
    ...
    static func resolveBranchProofs(node: MerkelNode, proof: Data = Data(), depth: Int = 0) throws -> [Proof] {
    ...
    static func hashBranch(left: MerkelNode, right: MerkelNode? = nil) -> MerkelNode {
        if right == nil {
            return left
        }
