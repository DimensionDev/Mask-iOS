...
public class PanModalPresentationAnimator: NSObject {
    ...
    private func animateDismissal(transitionContext: UIViewControllerContextTransitioning) {
        //print("[red-packet] animateDismissal1")
        guard
            let toVC = transitionContext.viewController(forKey: .to),
            let fromVC = transitionContext.viewController(forKey: .from)
            else { return }

        //print("[red-packet] animateDismissal2 \(fromVC.view.window)")
        // Calls viewWillAppear and viewWillDisappear
        ...
        PanModalAnimator.animate({
            ...
            //print("[red-packet] animateDismissal31 \(container) \(container.superview)")
            transitionContext.completeTransition(didComplete)
            //print("[red-packet] animateDismissal41 \(container.superview)")
            if container.superview != nil {
                //print("[red-packet] something wrong, hasn't removed the transition view")
