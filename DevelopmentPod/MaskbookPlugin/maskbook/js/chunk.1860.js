(globalThis["webpackChunk_masknet_extension"] = globalThis["webpackChunk_masknet_extension"] || []).push([[1860,451],{
...
var constants = __webpack_require__(22407);
...
;// CONCATENATED MODULE: ./src/network/gun/version.2/gun.patch.ts
...
// EXTERNAL MODULE: ./src/network/gun/version.2/hash.ts
var hash = __webpack_require__(21959);
...
const NFTAvatarGUN = gun2_gun2.get(constants/* NFT_AVATAR_DB_NAME */.$s);
...
async function getDataFromGUN(userId) {
    return NFTAvatarGUN// @ts-expect-error
    .get(userId).then();
}
...
async function getNFTAvatarFromRSS(userId, address) {
    let v = rss3_cache.get(address);
    if (!v || Date.now() > v[1]) {
        rss3_cache.set(address, [
            _getNFTAvatarFromRSS(userId, address),
            (0,addSeconds/* default */.Z)(Date.now(), 60).getTime()
        ]);
    }
    v = rss3_cache.get(address);
    const result = await (v === null || v === void 0 ? void 0 : v[0]);
    return result === null || result === void 0 ? void 0 : result.nft;
}
async function _getNFTAvatarFromRSS(userId, address) {
    ...
    const nfts = await web3_providers_src/* RSS3.getFileData */.S5.getFileData(rss, address, '_nfts');
    if (nfts) {
        return nfts[userId];
    }
    return web3_providers_src/* RSS3.getFileData */.S5.getFileData(rss, address, '_nft');
}
async function saveNFTAvatarToRSS(address, nft, signature) {
    const rss = web3_providers_src/* RSS3.createRSS3 */.S5.createRSS3(address, async (message)=>{
        return (0,EthereumService.personalSign)(message, address);
    });
    let _nfts = await web3_providers_src/* RSS3.getFileData */.S5.getFileData(rss, address, '_nfts');
    if (!_nfts) {
        _nfts = {
            [nft.userId]: {
                signature,
    ...
    await web3_providers_src/* RSS3.setFileData */.S5.setFileData(rss, address, '_nfts', _nfts);
    // clear cache
...
async function getNFTAvatar(userId, network, networkPluginId, chainId) {
...
async function saveNFTAvatar(address, nft, network, networkPluginId, chainId) {
    try {
