"use strict";
(globalThis["webpackChunk_masknet_extension"] = globalThis["webpackChunk_masknet_extension"] || []).push([[8777],{
 ...
 */ function getProfilePageUrlAtFacebook(user) {
    ...
    if (!isValidFacebookUsername(username)) throw new TypeError(locales_legacy/* i18n.t */.a.t('service_username_invalid'));
...
;// CONCATENATED MODULE: ./src/social-network-adaptor/facebook.com/shared.ts
...
const facebookShared = {
    ...
    utils: {
        ...
        getPostURL,
        share (message) {
            (0,src/* openWindow */.xw)(facebook_com/* FacebookAdaptor.getShareLinkURL */.m.getShareLinkURL?.(message));
        },
...
;
const searchUserIdSelector = ()=>querySelector('div[role="tablist"][data-visualcompletion="ignore-dynamic"] a[role="tab"]')
;
...
;
const searchFacebookEditProfileSelector = ()=>querySelector('[role="main"] [role="button"] [role="img"]').closest(1).querySelector('i[data-visualcompletion="css-img"]')
;
...
;
// #region
const toolBoxInSideBarSelector = ()=>/* cspell:disable-next-line */ querySelector('#ssrb_left_rail_start').closest(1).querySelector('h2').closest(1).querySelector('div > div > div > :nth-child(2) > ul > li:nth-child(2)')
;
 ...
 */ function getProfileIdentifierAtFacebook(links, allowCollectInfo) {
    ...
    try {
        ...
        const identifier = shared_base_src/* ProfileIdentifier.of */.WO.of(shared_base_src/* EnhanceableSite.Facebook */.Jk.Facebook, id).unwrapOr(null);
        if (identifier) {
            ...
            try {
                ...
                if (allowCollectInfo && image.getAttribute('aria-label') === nickname && nickname) {
                    service/* default.Identity.updateProfileInfo */.ZP.Identity.updateProfileInfo(identifier, {
                        nickname,
                        avatarURL: image.src
                    });
                    if (currentProfile?.linkedPersona) {
                        service/* default.Identity.createNewRelation */.ZP.Identity.createNewRelation(identifier, currentProfile.linkedPersona);
                    }
                }
            } catch  {}
            try {
...
const getBioDescription = ()=>{
    ...
    if (intro && node) {
        ...
        settings/* bioDescription */.JE[shared_base_src/* EnhanceableSite.Facebook */.Jk.Facebook].value = text;
    } else if (intro) {
        settings/* bioDescription */.JE[shared_base_src/* EnhanceableSite.Facebook */.Jk.Facebook].value = '';
    }
    return settings/* bioDescription */.JE[shared_base_src/* EnhanceableSite.Facebook */.Jk.Facebook].value;
};
const getFacebookId = ()=>{
...
const getPersonalHomepage = ()=>{
    ...
    if (intro && node) {
        ...
        settings/* personalHomepage */.yO[shared_base_src/* EnhanceableSite.Facebook */.Jk.Facebook].value = text;
    } else if (intro) {
        settings/* personalHomepage */.yO[shared_base_src/* EnhanceableSite.Facebook */.Jk.Facebook].value = '';
    }
    return settings/* personalHomepage */.yO[shared_base_src/* EnhanceableSite.Facebook */.Jk.Facebook].value;
};
...
function resolveLastRecognizedIdentityFacebookInner(ref, signal) {
    ...
    new umd.MutationObserverWatcher(self).addListener('onAdd', (e)=>assign(e.value)
    ...
    ).then(getUserID).then((id)=>assign({
            ...ref.value,
            identifier: shared_base_src/* ProfileIdentifier.of */.WO.of(shared_base_src/* EnhanceableSite.Facebook */.Jk.Facebook, id).unwrapOr(undefined)
        })
    );
}
function resolveCurrentVisitingIdentityInner(ref, cancel) {
    ...
    const assign = async ()=>{
        ...
        ref.value = {
            identifier: shared_base_src/* ProfileIdentifier.of */.WO.of(shared_base_src/* EnhanceableSite.Facebook */.Jk.Facebook, handle).unwrapOr(undefined),
            nickname,
            avatar,
            bio,
            homepage
...
function injectCompositionFacebook(signal) {
    ...
    signal.addEventListener('abort', shared_base_src/* CrossIsolationMessages.events.requestComposition.on */.WF.events.requestComposition.on((data)=>{
...
function UI() {
    ...
    const onHintButtonClicked = (0,react.useCallback)(()=>shared_base_src/* CrossIsolationMessages.events.requestComposition.sendToLocal */.WF.events.requestComposition.sendToLocal({
            reason: 'popup',
            open: true
        })
    , []);
...
var Comments = __webpack_require__(91632);
...
var CommentBox = __webpack_require__(27531);
...
const PostProviderFacebook = {
    posts: utils/* creator.EmptyPostProviderState */.D.EmptyPostProviderState(),
    start (signal) {
...
const PaletteModeProviderFacebook = {
    current: (0,shared_base_src/* createSubscriptionFromValueRef */._H)(currentTheme),
...
function ToolboxAtFacebook(props) {
    ...
    return /*#__PURE__*/ (0,jsx_runtime.jsx)(ToolboxUnstyled/* ToolboxHintUnstyled */.z, {
        ...
        Container: ({ children  })=>/*#__PURE__*/ (0,jsx_runtime.jsx)(Container, {
        ...
        ListItemButton: Item,
        Typography: Text,
        ListItemIcon: ({ children  })=>/*#__PURE__*/ (0,jsx_runtime.jsx)(Icon, {
                hasTopNavBar: props.hasTopNavBar,
                children: children
            })
        ,
...
function injectToolboxHintAtFacebook(signal, category) {
    ...
    (0,utils_watcher/* startWatch */.f)(watcher, signal);
    const hasTopNavBar = Boolean(document.querySelector('#ssrb_top_nav_start ~ [role="banner"]'));
    (0,renderInShadowRoot/* createReactRootShadowed */.o)(watcher.firstDOMProxy.afterShadow, {
        signal
...
function NFTAvatarInFacebookFirstStep() {
    ...
    const onChange = (0,react.useCallback)(async (info)=>{
        ...
        src_utils/* MaskMessages.events.NFTAvatarUpdated.sendToLocal */.ql.events.NFTAvatarUpdated.sendToLocal({
            ...
            pluginId: info.pluginId ?? base_src/* NetworkPluginID.PLUGIN_EVM */.FF.PLUGIN_EVM,
...
function NFTAvatarListInFaceBookMobile() {
    ...
    const onChange = (0,react.useCallback)(async (info)=>{
        ...
        identity.identifier && shared/* InMemoryStorages.FacebookNFTEventOnMobile.storage.userId.setValue */.uU.FacebookNFTEventOnMobile.storage.userId.setValue(identity.identifier.userId);
        shared/* InMemoryStorages.FacebookNFTEventOnMobile.storage.address.setValue */.uU.FacebookNFTEventOnMobile.storage.address.setValue(info.token.contract?.address);
        shared/* InMemoryStorages.FacebookNFTEventOnMobile.storage.tokenId.setValue */.uU.FacebookNFTEventOnMobile.storage.tokenId.setValue(info.token.tokenId);
        shared/* InMemoryStorages.FacebookNFTEventOnMobile.storage.pluginId.setValue */.uU.FacebookNFTEventOnMobile.storage.pluginId.setValue(info.pluginId ?? base_src/* NetworkPluginID.PLUGIN_EVM */.FF.PLUGIN_EVM);
        shared/* InMemoryStorages.FacebookNFTEventOnMobile.storage.chainId.setValue */.uU.FacebookNFTEventOnMobile.storage.chainId.setValue(info.token.chainId ?? types/* ChainId.Mainnet */.a_.Mainnet);
        shared/* InMemoryStorages.FacebookNFTEventOnMobile.storage.schema.setValue */.uU.FacebookNFTEventOnMobile.storage.schema.setValue(info.token.schema ?? types/* SchemaType.ERC721 */.XQ.ERC721);
    }, [
        identity
    ]);
    return /*#__PURE__*/ (0,jsx_runtime.jsx)(NFTAvatar/* NFTAvatar */.R, {
...
const clearStorages = ()=>{
    shared/* InMemoryStorages.FacebookNFTEventOnMobile.storage.userId.setValue */.uU.FacebookNFTEventOnMobile.storage.userId.setValue('');
    shared/* InMemoryStorages.FacebookNFTEventOnMobile.storage.address.setValue */.uU.FacebookNFTEventOnMobile.storage.address.setValue('');
    shared/* InMemoryStorages.FacebookNFTEventOnMobile.storage.tokenId.setValue */.uU.FacebookNFTEventOnMobile.storage.tokenId.setValue('');
};
function NFTAvatarInFacebook() {
    ...
    const { value: _avatar  } = (0,hooks/* useNFTAvatar */.iF)(identity.identifier?.userId, constants/* RSS3_KEY_SNS.FACEBOOK */.vu.FACEBOOK);
    ...
    (0,useAsync/* default */.Z)(async ()=>{
        const storages = shared/* InMemoryStorages.FacebookNFTEventOnMobile.storage */.uU.FacebookNFTEventOnMobile.storage;
        if (!account) return;
        if (!identity.identifier) return;
...
// EXTERNAL MODULE: ../../node_modules/.pnpm/@mui+material@5.8.0_4xriuibr7qayay6mzcpq5f43me/node_modules/@mui/material/utils/debounce.js
var debounce = __webpack_require__(30545);
...
const facebookUI = {
    ...
    init (signal) {
        const profiles = utils/* stateCreator.profiles */.Z.profiles();
    ...
    configuration: {
        steganography: {
            ...
            password () {
                const id = IdentityProviderFacebook.recognized.value.identifier?.userId || social_network/* globalUIState.profiles.value */.EJ.profiles.value?.[0].identifier.userId;
                if (!id) throw new Error('Cannot figure out password');
                return shared_base_src/* ProfileIdentifier.of */.WO.of(shared_base_src/* EnhanceableSite.Facebook */.Jk.Facebook, id).unwrap().toText();
            }
        }
    }
};
