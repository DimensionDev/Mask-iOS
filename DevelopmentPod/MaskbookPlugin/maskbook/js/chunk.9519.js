(globalThis["webpackChunk_masknet_extension"] = globalThis["webpackChunk_masknet_extension"] || []).push([[9519],{
...
`;
async function fetchFromNFTRedPacketSubgraph(chainId, query) {
    const subgraphURL = (0,constants/* getNftRedPacketConstants */.pS)(chainId).SUBGRAPH_URL;
...
async function getNftRedPacketTxid(chainId, rpid) {
    const data = await fetchFromNFTRedPacketSubgraph(chainId, `
    {
...
const PAGE_SIZE = 5;
async function getNftRedPacketHistory(chainId, address, page) {
    const data = await fetchFromNFTRedPacketSubgraph(chainId, `
    {
        nftredPackets (
    ...
    return data.nftredPackets.map((x)=>{
        const nftRedPacketSubgraphInMask = {
            ...
            token_contract: (0,helpers/* tokenIntoMask */.F6)(x.token_contract),
        ...
        const network = resolver/* chainResolver.chainNetworkType */.VU.chainNetworkType(nftRedPacketSubgraphInMask.chain_id);
        const sender = {
...
`;
async function fetchFromRedPacketSubgraph(chainId, query) {
    const { SUBGRAPH_URL  } = (0,constants/* getRedPacketConstants */.GV)(chainId);
...
async function getRedPacketTxid(chainId, rpid) {
    const data = await fetchFromRedPacketSubgraph(chainId, `
    {
...
async function getRedPacketHistory(chainId, address) {
...
// EXTERNAL MODULE: ./src/plugins/RedPacket/Worker/database.ts
var database = __webpack_require__(19057);
// EXTERNAL MODULE: ../../node_modules/.pnpm/lodash-es@4.17.21/node_modules/lodash-es/omit.js + 3 modules
var omit = __webpack_require__(96596);
...
async function getRedPacketNft(id) {
    const record = await database/* RedPacketDatabase.get */.Uy.get('red-packet-nft', id);
    return record ? RedPacketNftRecordOutDB(record) : undefined;
}
async function addRedPacketNft(record) {
    return database/* RedPacketDatabase.add */.Uy.add(RedPacketNftRecordIntoDB(record));
}
async function updateRedPacketNft(newRecord) {
    if (!newRecord.id) {
        return;
    }
    const record = await database/* RedPacketDatabase.get */.Uy.get('red-packet-nft', newRecord.id);
    if (record) {
        await database/* RedPacketDatabase.remove */.Uy.remove('red-packet-nft', newRecord.id);
    }
    await database/* RedPacketDatabase.add */.Uy.add(newRecord);
}
...
async function discoverRedPacket(record, chainId) {
    if (record.contract_version === 1) {
        const txid = await getRedPacketTxid(chainId, record.id);
        if (!txid) return;
        record.id = txid;
    }
    database/* addRedPacket */.np(record);
}
...
async function services_getNftRedPacketHistory(address, chainId, page) {
    const histories = await getNftRedPacketHistory(chainId, address, page);
