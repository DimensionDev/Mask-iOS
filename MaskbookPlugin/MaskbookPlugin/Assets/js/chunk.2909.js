"use strict";
(globalThis["webpackChunk_masknet_extension"] = globalThis["webpackChunk_masknet_extension"] || []).push([[9255],{
...
var exponent = __webpack_require__(31812);
...
/* harmony export */   "Z": () => (/* export default binding */ __WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony import */ var _exponent_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(31812);
...
function useLineChart(svgRef, data, dimension, id, opts) {
    ...
    (0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{
        ...
        const y = d3__WEBPACK_IMPORTED_MODULE_0__/* .scaleLinear */ .BYU().domain([
            min - dist * 0.05,
            max + dist * 0.05
        ]).range([
            contentHeight,
        ...
        // add Y axis
        graph.append('g').attr('transform', 'translate(0, 0)').call(d3__WEBPACK_IMPORTED_MODULE_0__/* .axisRight */ .Khx(y).ticks(contentHeight / 50, tickFormat).tickSize(contentWidth)).call((g)=>g.select('.domain').remove()
        ...
        const callout = (g, value)=>{
            ...
            const text = g.selectAll('text').data([
                null
            ]).join('text').call((text1)=>text1.selectAll('tspan').data((value + '').split(/\n/)).join('tspan').attr('x', 0).attr('y', (d, i)=>`${i * 1.1}em`
                ).style('font-weight', (_, i)=>i ? null : 'bold'
                ).text((d)=>d
                )
            );
            const textBBox = (ref = text.node()) === null || ref === void 0 ? void 0 : ref.getBBox();
            if (textBBox) {
                const { x: x1 , y: y1 , width: w , height: h  } = textBBox;
                text.attr('transform', `translate(${-w / 2},${15 - y1})`);
        ...
        d3__WEBPACK_IMPORTED_MODULE_0__/* .select */ .Ys(svgRef.current).on('mousemove', function() {
            ...
            const mx = d3__WEBPACK_IMPORTED_MODULE_0__/* .mouse */ .Jzs(this)[0];
            ...
            const bisect = (mx1)=>{
                const date = x.invert(mx1);
                const index = d3__WEBPACK_IMPORTED_MODULE_0__/* .bisector */ .YFb((d)=>d.date
                ).left(data, date, 1);
                return data[index];
            };
            const { date , value  } = bisect(fixedX);
            tooltip.attr('transform', `translate(${Number(x(date))},${y(value)})`).call(callout, `${formatTooltip(value)}
